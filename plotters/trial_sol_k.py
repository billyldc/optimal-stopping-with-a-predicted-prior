...
from opt_analytical_threshold import search_optimal_K
...
if __name__ == "__main__":
    ...
    betas = np.linspace(10e-6, 1.0 / np.e - 5e-3, 100)
    # for beta in betas:
    #     plot_J(beta=beta, K=1.71, n_v=401)

    #use to omit the extreme values
    ignorance=0.005


    results = []
    for beta in betas:
        res = find_best_K_for_beta(beta, K_grid=np.linspace(0.1, 2, 100), n_v=200, ignorance=ignorance)
        print(
            f"beta={res['beta']:.6g}, lambda1={res['lambda1']:.6g}, lambda2={res['lambda2']:.6g}, "
            f"best_K={res['best_K']:.6g}, best_min_value={res['best_value']:.6g}, u_at_min={res['u_at_min']:.6g}"
        )
        results.append(res)
    
    # plot alpha,beta
    alpha_values = [res['best_value']-ignorance*2 for res in results]
    plt.plot(alpha_values, betas, '-', label="our algorithm with ignorance="+str(ignorance))
    plt.ylabel('beta')
    plt.xlabel('alpha')
    plt.title('Alpha vs Beta with ignorance='+str(ignorance))
