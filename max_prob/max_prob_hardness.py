...
class Distributions:
    ...
    def __init__(self, probs, values=None, indices=None, prior_index=None):
        ...
        if indices is not None and not all(
            isinstance(index, int) and 0 <= index < len(probs) for index in indices
        ):
            raise ValueError(
                "Indices must be nonnegative integers between 0 and len(probs) - 1."
            )

        if prior_index is not None and not (
            isinstance(prior_index, int) and 0 <= prior_index < len(probs)
        ):
            raise ValueError(
                "Prior index must be an integer between 0 and len(probs) - 1."
            )
...
class LP_maxprob:
    ...
    def __init__(self, n, F, alpha, beta, outputflag=0, monotone=False):
        ...
        if not (
            (alpha is None and isinstance(beta, (float, int)) and 0 <= beta <= 1)
            or (isinstance(alpha, (float, int)) and 0 <= alpha <= 1 and beta is None)
        ):
            raise ValueError(
                "Exactly one of alpha and beta must be None. The other one must be a float or int between 0 and 1."
            )
        self.n = n
        self.F = F
        self.model = Model()
    ...
    def build(self, alpha, beta, outputflag=0):
        ...
        self.S = self.model.addVars(
            list(product(range(self.n + 1), range(self.F.K))), lb=0, ub=1, name="S"
        )
        ...
        self.model.addConstr(
            self.alpha <= self.ALG(self.F.k_star, self.S),
            name=f"consistency_{self.F.k_star}",
        )

        # Add robustness constraint for the prior distribution
        for k in self.F.indices:
            self.model.addConstr(
                self.beta <= self.ALG(k, self.S), name=f"robustness_{k}"
            )
    ...
    def ALG(self, k, S):
        ...
        return c ** (self.n) * sum(
            sum(
                ...
                * (
                    sum(
                        S[t - 1, m] * self.F.P(t - 1, m, "==")
                        for m in range(l, self.F.K)
                    )
                    * self.F.P(1, l, "==")
                    + S[t - 1, l] * self.F.P(t - 1, l, "==") * self.F.P(1, l + 1, ">=")
                    - S[t, l] * self.F.P(t, l, "==")
                )
...
def experiment(n, p, density, simplify=False):
    ...
    Alpha = np.linspace(
        min_alpha, max_alpha, int((max_alpha - min_alpha) / density) + 2
    )
    ...
    if use_parallel:
        ...
        args_list = [
            (alpha_id, alpha, n, p, None) for alpha_id, alpha in enumerate(Alpha)
        ]
        ...
        print(
            f"Parallel processing completed. {len(Alpha_)} successful solutions out of {len(Alpha)} total."
        )
    else:
...
class alpha_beta_curve:
    ...
    def __init__(
        ...
        if n is None or m is None:
            raise ValueError(
                "When datafile is not provided, both n and m must be given."
            )
        self.n = n
        self.m = m
        ...
        if load_from_file is True:
            ...
            print(
                f"An experiment with n={n}, m={m}, p={strp}, density={density} is running ..."
            )
            self.Alpha, self.Beta = experiment(
                self.n, self.p, density, simplify=simplify
            )
    ...
    def plot(
        self, savefig=True, xaxisrange=(0, 1), yaxisrange=(0, 1), add_legend=False
    ):
        # plot
