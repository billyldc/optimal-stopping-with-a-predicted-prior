...
def solve_C_for_K(
    K, u0=0.0, u1=1.0, method="Radau", rtol=1e-10, atol=1e-10, n_points=200
):
    """
...
def search_optimal_K(
    u0=0.0, u1=1.0, K_start=0.01, K_end=1.0, max_iter=10, n_points=5000
):
    """
    ...
    print(
        f"Search done in {it} iters. Largest failing K ~ {K_low}, smallest reaching K ~ {K_high}"
    )
    return (K_low, K_high)


def plot_one_K(
    K_value, u0=0.0, u1=1.0, is_labeled=True, plotter=None, n_points=100, rounding=10
):
...
def plot_batch(K_u_label_tuples, ylimit=(0, 20), num_points=100, rounding=10):
    ...
    for K, u0, u1, is_labeled in K_u_label_tuples:
        plotter = plot_one_K(
            K, u0, u1, is_labeled, plotter, n_points=num_points, rounding=rounding
        )
    plt.xlim(0, 1)
    plt.ylim(ylimit)
    plt.xlabel("u")
...
def compute_min_ratio(
    ...
    def integral_I(z):
        ...
        try:
            val, _ = quad(
                lambda t: (np.exp(-t * C) - np.exp(-t * z)) / (t * t),
                lambda2,
                1.0,
                epsabs=quad_eps,
                epsrel=quad_eps,
    ...
    print(
        f"Modified compute_min_ratio: found root z={z_root} with H(z)={H(z_root)} after {iterations} iterations."
    )
...
def optimal_alpha_beta(density):
    ...
    for beta in np.linspace(start, end, n_points):
        ...
        K_low, K_high = search_optimal_K(
            u0=lambda1, u1=lambda2, K_start=0.01, K_end=1.0, max_iter=20
        )
        print(
            f"beta={beta}, lambda1={lambda1}, lambda2={lambda2}, K_low={K_low}, K_high={K_high}"
        )
...
if __name__ == "__main__":
    ...
    for beta in betas:
        ...
        K_low, K_high = search_optimal_K(
            u0=lambda1, u1=lambda2, K_start=0.01, K_end=1.0, max_iter=20, n_points=1000
        )
        alpha_1 = K_high if K_high is not None else np.nan
        alphas_1.append(alpha_1)
        sol, reached_full = solve_C_for_K(
            alpha_1, u0=lambda1, u1=lambda2, n_points=1000
        )
        C1 = sol.y[0][-1] if sol is not None and sol.y.shape[1] > 0 else None
        # compute alpha_2 from alpha_1, C1
        u, alpha2 = compute_min_ratio(
            alpha=alpha_1, C=C1, lambda2=lambda2, z_max=1000.0, if_plot=False
        )
        alphas_2.append(alpha2)
        print(
            f"{beta:.5f}, {lambda1:.5f}, {lambda2:.5f}, {alpha_1:.5f}, {C1:.5f}, {u:.5f}, {alpha2:.5f}"
        )
